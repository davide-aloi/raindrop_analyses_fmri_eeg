%% This script runs a hierarchical PEB of pre < post differences in each session
% Davinia Fernandez-Espejo 8th July 2020
% Adapted by Davide Aloi (15/07/2022) for p01 raindrop

%% Edit the parameters below
clearvars;
path = 'C:\Users\davide\Documents\GitHub\raindrop_analyses_fmri_eeg\fmri\DCM\dcm_results_p01\'; % path where you have the estimated GCM file
cd(path)
load('GCM_fullABC.mat')

% fields
field = {'A'
         %'B'
         }; 

%% specify settings for PEB day 1 < day 5
Mpp = struct();
Mpp.Q     = 'all';
Mpp.Xnames = {'constant', 'pre<post'};
Mpp.X(:,1)=[1 1];
Mpp.X(:,2)= [-1 1];
Mpp.maxit = 256; % convergence before max num of iterations is reached

%% conditions for the pebs
polar = {'sham' 
    'anodal'
    'cathodal'};
%% Loop
N = 12; %n of sessions
% sessions
tot_sess = length(GCM)/2; % total pre post comparisons

%% loop
for fi = 1:length(field)
    PEBpp=cell(tot_sess,1);
    GCMpp=cell(tot_sess,1);
    BMA=cell(length(polar),1);
    sess = 1;
    
    for sub = 1:tot_sess
        GCM_temp = GCM(sess:sess+1);
        [PEBpp{sub},GCMpp{sub}] = spm_dcm_peb(GCM_temp,Mpp,field(fi));
        BMA{sub} = spm_dcm_peb_bmc(PEBpp{sub});
        sess=sess+2;
    end
    save(strcat('PEBpp',field{fi}),'GCMpp','PEBpp');
    save(strcat('PEBpp_bma',field{fi}),'BMA');
end